...
if not os.path.exists(ALARM_FILE):
    pd.DataFrame(columns=["ts","source","severity","message"]).to_csv(ALARM_FILE,index=False)

def raise_alarm(source, severity, message, ts):
    entry = {"ts": ts.isoformat(), "source": source,
             "severity": severity, "message": message}
    df = pd.read_csv(ALARM_FILE) if os.path.exists(ALARM_FILE) else pd.DataFrame(columns=entry.keys())
    if not df.empty and str(df.iloc[-1].get("source"))==source and str(df.iloc[-1].get("message"))==message:
        return
    df = pd.concat([df, pd.DataFrame([entry])], ignore_index=True)
    df.to_csv(ALARM_FILE, index=False)
